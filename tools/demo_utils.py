...
def view_points(points: np.ndarray, view: np.ndarray, normalize: bool) -> np.ndarray:
    ...
    """
    # 将3D点投影到2D平面上, 支持透视和正交投影, 可用于点云的可视化
    ...
    # Do operation in homogenous coordinates. 在齐次坐标系下做运算
...
def _second_det_to_nusc_box(detection):
    # 将检测结果转换为nuscenes格式. 每个框由位置、大小、朝向、标签、得分和速度等属性定义
...
class Box:
    ...
    # 代表一个3D框，包含中心位置、大小、朝向、标签、得分、速度以及可选的名称和唯一标识符
    def __init__(self,
                 center: List[float],
                 size: List[float],
